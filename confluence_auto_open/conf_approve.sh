...
JQ="/usr/local/bin/jq"
#about dir
SCRIPTPATH="/opt/script/conf_approve"
...
then
/usr/bin/python $ADDUSER $NEWUSER $PASSWD
echo " $TIME_STAMP
您的Confluence账号已开通，用户名是姓名全拼，密码是：$PASSWD
访问地址：http://2.2.2.2:8080
" | mail -s "账号开通成功" $NEWUSER@666.com
echo "$TIME_STAMP $NEWUSER 账号成功开通。" >> $USER_LOG
echo $i >> $CONFBUSSLIST
$mysql -h$h -u$u -p$p -D$d -e  "insert into confluence (confluence) value (\"$NEWUSER\");"
